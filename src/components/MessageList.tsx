  ...
  return (
    <div className="flex-1 flex flex-col relative min-h-0 overflow-hidden">
      ...
      {showScrollTop && (
        <button
          onClick={scrollToTop}
          className="absolute top-4 right-4 z-20 p-2 bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-full shadow-md text-neutral-500 hover:text-primary dark:hover:text-secondary-light transition-all hover:scale-110"
          title="Scroll to top"
        >
      ...
      <div ref={scrollContainerRef} onScroll={checkScroll} className="flex-1 overflow-y-auto p-3 lg:p-4 no-scrollbar scroll-smooth">
        {models.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-center pb-20 animate-in fade-in zoom-in duration-500">
            ...
            <button onClick={() => { setWizardStep(3); setShowModelWizard(true); setIsModelDropdownOpen(false); }} className="px-8 py-4 bg-secondary-light hover:bg-secondary-muted text-primary-dark rounded-2xl font-black text-sm uppercase tracking-widest shadow-xl shadow-secondary-light/30 transition-all active:scale-95 flex items-center gap-3"><Plus className="w-5 h-5" /> Connect Your First LLM</button>
          </div>
          ...
          <div className="h-full flex flex-col items-center justify-center text-center opacity-40 pointer-events-none pb-20">
            <AgentIcon agent={activeAssistant} sizeClass="w-16 h-16" containerClass="p-4 rounded-3xl mb-4" />
            <h2 className="text-3xl font-black italic tracking-tighter uppercase">Start Session</h2>
            {activeAssistant?.description && <p className="text-sm font-medium mt-2 max-w-xs">{activeAssistant.description}</p>}
            <p className="text-tiny font-medium text-neutral-400 mt-3 uppercase tracking-widest">Type anything to begin</p>
          </div>
        )}
      </div>
    </div>

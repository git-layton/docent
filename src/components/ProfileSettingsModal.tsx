            ...
            <div className="space-y-6">
              ...
              <div className="p-5 rounded-2xl border border-edge bg-panel flex flex-col gap-5">
                ...
                <div className="flex items-center justify-between gap-4 border-t border-edge pt-5">
                  ...
                  <button onClick={() => setAppSettings((prev: any) => ({ ...prev, frostedPanel: !prev.frostedPanel }))} className={`w-10 h-5 rounded-full transition-all relative shrink-0 ${appSettings.frostedPanel ? 'bg-primary' : 'bg-inset'}`}>
                    <div className={`absolute top-0.5 w-4 h-4 rounded-full bg-white transition-all ${appSettings.frostedPanel ? 'right-0.5' : 'left-0.5'}`} />
                  </button>
                </div>
